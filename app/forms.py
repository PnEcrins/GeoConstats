...
class ConstatForm(FlaskForm):
    ...
    nom_agent1 = StringField(
        ...
        [
            RequiredIf(
                "declaratif",
                message="Veuillez fournir le nom d'au moin un agent",
            )
        ],
    )
...
def get_comm():
    return (
        LAreas.query.filter_by(id_type=25)
        .filter(Constats.id_commune == LAreas.id_area)
        .all()
    )
...
def get_secteur():
    return (
        LAreas.query.filter_by(id_type=30)
        .filter(Constats.id_secteur == LAreas.id_area)
        .all()
    )
...
class FilterForm(FlaskForm):
    ...
    type_constat = SelectField(
        "Type de constat",
        choices=[(None, "Tous"), (0, "Constat"), (1, "Constat déclaratif")],
    )
    ...
    localisation = SelectField(
        "Localisation du constat",
        choices=[
            (0, ""),
            (1, "Coeur du parc"),
            (2, "Dans l'aire d'adhésion"),
            (3, "Hors du parc"),
        ],
    )
