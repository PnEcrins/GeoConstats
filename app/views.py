...
def map(id_role):
    ...
    if "date" in filter_query:
        if filter_query["date"] != "0":
            query = query.filter(
                extract("year", Constats.date_constat) == int(filter_query["date"])
            )
    ...
    if "localisation" in filter_query:
        if filter_query["localisation"] == "1":
            ...
            query = query.filter(
                Constats.dans_aa == False and Constats.dans_coeur == False
            )
...
def add(id_role):
    ...
    p2154 = DB.session.query(
        func.ST_AsGeoJson(
            func.ST_Transform(
                func.ST_SetSRID(
                    func.ST_Point(float(data["geomlng"]), float(data["geomlat"])), 4326
                ),
                2154,
            )
        )
    ...
    constat = Constats(
        ...
        the_geom_point=from_shape(
            Point(json2154["coordinates"][0], json2154["coordinates"][1]), srid=2154
        ),
        id_role=id_role,
    ...
    if form.id_constat.data:
        ...
        nivDroit = (
            ...
            .filter(AppUser.id_application == app.id_application)
            .one()[0]
        )
...
def delete(idc, id_role):
    ...
    nivDroit = (
        DB.session.query(AppUser.id_droit_max)
        .filter(AppUser.id_role == id_role)
        .filter(AppUser.id_application == dataApp[0])
        .one()
    )
    if nivDroit[0] > 2 or id_role == dataGeom.id_role:
        dataGeom = (
            DB.session.query(Constats).filter(Constats.id_constat == idc).delete()
        )
        DB.session.commit()
        return redirect(url_for("routes.map"))
    else:
...
def download(id_role):
    ...
    schema = ConstatSchemaDownload(
        exclude=["id_secteur", "id_commune", "id_departement", "id_role"]
    )
    if "date" in filter_query:
        if filter_query["date"] != "0":
            query = query.filter(
...
def dashboard(id_role):
    ...
    nb_constat_by_sect = (
        ...
        .select_from(LAreas)
        .filter(LAreas.id_type == 30)
        .group_by(LAreas.area_name)
    )

    query_constat_by_sect_by_animal_type = (
        DB.session.query(
    ...
    nb_constat_by_dep = (
        nb_constat_by_dep.outerjoin(
            Constats, and_(Constats.id_departement == LAreas.id_area, *filters)
        )
        .group_by(LAreas.area_name)
        .all()
    )
    nb_constat_by_sect = (
        nb_constat_by_sect.outerjoin(
            Constats, and_(Constats.id_secteur == LAreas.id_area, *filters)
        )
        .group_by(LAreas.area_name)
        .all()
    )
    ...
    data = (
        query_constat_by_sect_by_animal_type.outerjoin(
            Constats, and_(Constats.id_secteur == LAreas.id_area, *filters)
        )
        .outerjoin(bib_type_animaux, bib_type_animaux.id == Constats.type_animaux)
...
def get_areas():
    ...
    data = (
        DB.session.query(
            LAreas.area_name,
            func.st_asgeojson(
                func.st_transform(func.st_simplify(LAreas.geom, 10), 4326),
            ),
        )
        .join(BibAreaType, BibAreaType.id_type == LAreas.id_type)
        .filter(BibAreaType.type_code == area_type_code)
        .all()
    )
